...
router.get('/', (q,a)=>{
    ExchangeRate.find(q.param.currencyCode)
        .then(exchangeRates => a.json(exchangeRates));
});

//find and update one rate by id
router.put('/:id',(q,a)=>{
    ExchangeRate.findByIdAndUpdate(q.params.id, q.body)
        .then(a.json(post));
});
...
router.delete('/:id',(q,a)=>{
    ExchangeRate.findById(q.params.id)
        .then(exchangeRates => exchangeRates.remove().then(() => a.json({success :true})))
